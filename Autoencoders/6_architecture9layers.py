#This is the code for different layers-9 layers
#691-512-256-128-64-128-256-512-691
...
class AutoEncoder(nn.Module):
    def __init__(self,in_fea,h1,h2,h3,latent_size,d_output,drop_rate):
        super(AutoEncoder,self).__init__()

        self.l1 = nn.Linear(in_fea,h1)
        self.l2 = nn.Linear(h1,h2)
        self.l3 = nn.Linear(h2,h3)
        self.l4 = nn.Linear(h3,latent_size)
        self.l5 = nn.Linear(latent_size,h3)
        self.l6 = nn.Linear(h3,h2)
        self.l7 = nn.Linear(h2,h1)
        self.l8 = nn.Linear(h1,in_fea)
        
        self.l9 = nn.Linear(latent_size,d_output)
    ...
    def forward(self,x):
        ...
        x3 = self.drop(F.relu(self.l3(x2)))
        embedding = F.relu(self.l4(x3))
        x4 = F.relu(self.l5(embedding))
        x5 = F.relu(self.l6(x4))
        x6 = self.drop(F.relu(self.l7(x5)))      
        x7 = self.l8(x6)
        
        x8 = self.l9(embedding)
        return x7,embedding,x8
...
for i in range(5):
    ...
    for k in range(splits):
        ...
        for l_r in learning_rate:
            ...
            model = AutoEncoder(in_fea,h1,h2,h3,latent_size,d_output,drop_rate)
            model = model.double()
...
OUTPUT.to_csv('691-512-256-128-64-128-256-512-691.csv',sep='\t')
