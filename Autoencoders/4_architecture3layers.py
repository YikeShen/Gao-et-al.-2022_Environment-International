#This is the code for different layers-3 layers
#691-128-691
...
class AutoEncoder(nn.Module):
    def __init__(self,in_fea,latent_size,d_output,drop_rate):
        super(AutoEncoder,self).__init__()

        self.l1 = nn.Linear(in_fea,latent_size)
        self.l2 = nn.Linear(latent_size,in_fea)
        self.lpredict = nn.Linear(latent_size,d_output)
        
        self.drop = nn.Dropout(drop_rate)
        self.act = nn.Tanh()
    def forward(self,x):
        embedding = self.drop(F.relu(self.l1(x)))
        x2 = self.l2(embedding)
        xpredict = self.lpredict(embedding)
       
        return x2,embedding,xpredict
...
for i in range(5):
    ...
    for k in range(splits):
        ...
        for l_r in learning_rate:
            ...
            model = AutoEncoder(in_fea,latent_size,d_output,drop_rate)
            model = model.double()
...
OUTPUT.to_csv('691-128-691.csv',sep='\t')
